...
proc setup() : void {.exportC:"setup".} =
    ...
    if base.win.save_screenshot("/Volumes/btrfs/sdk/nim.test/nimsdk/out.bmp", "from pview"):
        echo "screenshot:ok"
    else:
        echo "screenshot:error"
        var
            result : Filename = base.win.saveScreenshotDefault("/Volumes/btrfs/sdk/nim.test/nimsdk/out")
        echo result
